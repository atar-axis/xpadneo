...
static inline int l2cap_config_req(struct l2cap_conn *conn,
				   ...
				   u8 *data)
{
	BT_INFO("Sending Config Request\n");
...
static inline int l2cap_config_rsp(struct l2cap_conn *conn,
				   ...
				   u8 *data)
{
	BT_INFO("Received Config Response\n");
	...
	case L2CAP_CONF_UNKNOWN:
		/* we sent an option which isn't supported by the other side */

		/* BT L2CAP Specification (p.284):
		 * 
		...
		/* there are at least two implementation problems:
		 * 1) we didn't handle L2CAP_CONF_UNKNOWN
		 * 2) if we already succeded a configuration, we should (maybe)
		 *    stop/ignore any further config responses/requests.
		 */

		BT_INFO("Host reported \"Unknown Config Options\"");
		print_hex_dump(KERN_INFO, "-> unknown option: ", DUMP_PREFIX_NONE, 16, 1, rsp->data, len, false);

		/* Xbox Controller does not know L2CAP_CONF_RFC
		 * therefore we let the switch fall through, since L2CAP_CONF_UNACCEPT
		 * does simply removes the unaccepted option (which is the same
		 * as we would do here) and starts a new configuration request
		 */

	case L2CAP_CONF_UNACCEPT:

		/* if we haven't reached the maximum configuration responses */
		if (chan->num_conf_rsp <= L2CAP_CONF_MAX_CONF_RSP) {

			/* create new request buffer */
			char req[64];

			/* check if the reponse length is bigger than our buffer */
			...
			/* parse the response data into our req array */
			/* original values are untouched if not mentioned in the response */
			...
			/* test if the new config is ok for us */
			if (result != L2CAP_CONF_SUCCESS)
				goto done;
			break;
		}
