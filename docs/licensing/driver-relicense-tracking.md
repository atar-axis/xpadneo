...
| Ben Schattinger | @lights0123 | Series X/S support; Share button quirks + HID bit fixups | 16 / 2 (driver) | non-trivial logic | request consent (preferred) OR rewrite | ⏳ | Adds quirk flag, usage map for Share (KEY_RECORD), raw_event bit re-mapping, adds PID 0x0B13. If no consent: re-implement share button handling + quirks mapping. |
| Erik Hajnal | @ehats | XBE2 unknown mode: mapping/fixups, ignore keyboard, reportsize 55 | 42 / 11 (driver) | non-trivial logic | request consent (preferred) OR rewrite | ⏳ | Adds many USAGE_IGN entries, ignores HID_UP_KEYBOARD, extends raw_event reportsize handling, tweaks PID handling. If no consent: re-derive mapping/fixups from descriptors and re-implement. |
| Dugan Chen | @duganchen | Elite Series 2 wireless: adds BT PID + comments + disabled (#if 0) hack | 21 / 2 (driver) | trivial-ish / data plumbing | rewrite/replace | ⏳ | Mostly PID table entry (0x0B05), comment expansion, and #if 0 stub. Safe to replace during refactor (new device-id tables). |
