...
static int ff_upload_effect(struct input_dev *dev, struct ff_effect *effect, struct ff_effect *old)
{
	...
	case FF_DAMPER:
		hid_info(hdev,
			 ...
			 effect->u.condition[0].right_coeff, effect->u.condition[0].left_coeff,
			 effect->u.condition[0].deadband, effect->u.condition[0].center);
		hid_info(hdev,
			 "ff: condition details (axis 1): r_sat %u, l_sat %u, r_coeff %d, l_coeff %d, deadband %u, center %d\n",
			 effect->u.condition[1].right_saturation,
			 effect->u.condition[1].left_saturation,
			 effect->u.condition[1].right_coeff, effect->u.condition[1].left_coeff,
			 effect->u.condition[1].deadband, effect->u.condition[1].center);
		break;
	}
