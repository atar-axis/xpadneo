...
static bool param_fake_old_pid = true;
module_param_named(fake_old_pid, param_fake_old_pid, bool, 0644);
MODULE_PARM_DESC(fake_old_pid, "(bool) Fake the product ID to 0x02E0, which is the old firmware pId.");
				    ...
				    struct hid_input *hi)
{
	...
	if (param_fake_old_pid) {
		xdata->idev->id.product = 0x02E0;
		hid_dbg_lvl(DBG_LVL_FEW, hdev, "Fake product id: 0x02E0 (old firmware)\n");
	}
