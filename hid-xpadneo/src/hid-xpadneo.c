...
static int xpadneo_input_configured(struct hid_device *hdev, struct hid_input *hi)
{
	...
	input_set_abs_params(xdata->idev, ABS_PROFILE, 0, XPADNEO_XBE2_PROFILES_MAX - 1, 0, 0);
	input_report_abs(xdata->idev, ABS_PROFILE, 0);
...
stop_processing:
	...
	if (xdata->profile >= XPADNEO_XBE2_PROFILES_MAX) {
		...
		input_report_abs(idev, ABS_PROFILE, xdata->profile);
